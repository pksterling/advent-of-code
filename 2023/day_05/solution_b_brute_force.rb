...
seed_instructions = almanac.shift.scan(/\d+/).map(&:to_i)
maps = almanac.map { |map_string| map_string.scan(/(?<=\n)[\d ]+/).map{ |string| string.scan(/\d+/).map(&:to_i) } }

seeds = seed_instructions.each_with_index.map do |number, index|
  Array(number..(number + seed_instructions[index + 1] - 1)) if index.even?
end.compact.flatten
